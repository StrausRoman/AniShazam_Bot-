...
def build_reply(results: list[dict]) -> str:
    ...
    total_checked = results[0].get("frames_checked", 0)
    top_votes = results[0].get("votes", 1)

    # Если ни одно аниме не подтвердилось больше чем на одном кадре из многих проверенных —
    # это не "нашли неправильно", а "не нашли вообще". trace.moe индексирует не всё аниме
    # и в таком случае просто выдаёт "ближайшее визуально похожее", а не честное "нет в базе".
    if top_votes < 2 and isinstance(total_checked, int) and total_checked >= 8:
        return (
            f"😕 Проверил {total_checked} кадров, но ни одно аниме не подтвердилось "
            "больше чем на одном кадре — совпадения каждый раз разные.\n\n"
            "Скорее всего, этой сцены просто нет в базе trace.moe (сервис индексирует "
            "не всё аниме — например, могут отсутствовать редкие тайтлы, ремастеры "
            "или совсем свежие эпизоды). В таком случае найти точный источник, увы, не получится."
        )
